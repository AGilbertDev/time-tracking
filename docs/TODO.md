...
- [ ] **No write path anywhere is atomic, and the reset makes that visible.** Raised on [#48](https://github.com/AGilbertDev/time-tracking/pull/48) against the onboarding reset, which deletes the settings row, deletes the quota rows and clears `onboarded_at` as three separate statements. A concurrent `completeOnboarding` can pass its `onboarded_at` guard after the reset clears the flag and then write settings the reset deletes a moment later, leaving a completed account with no settings and two sessions disagreeing about its onboarding state. The finding is correct and it is not specific to this endpoint. There is no `db.transaction` and no `db.batch` anywhere in `server/`, so every multi-statement write in the app has the same property, and the reset was written to be ordered and re-runnable precisely because atomicity was not available. Adding a transaction here alone would make this the only atomic path in the codebase, which is why it is recorded rather than done in the feature that surfaced it. libSQL supports batched statements, so the work is a pass across every multi-statement write plus a decision about whether the ordering-and-idempotency pattern stays as the fallback.
