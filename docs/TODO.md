...
- [ ] **`settings.quotas.userSince` renders `1 août` where French wants `1er août`.** `Intl` will not produce the ordinal, but `formatToParts` plus `Intl.PluralRules` with `type: 'ordinal'` will, since `fr-CA` returns `one` for 1 and `other` for 2, which is exactly the French rule.
