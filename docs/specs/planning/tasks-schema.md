...
1. **Status vocabulary (soft, `overview` open question 5).** The three trackable status names are `Accepté` / `En cours` / `Terminé`, with `N/A` for non-trackable, still to be confirmed with the primary user. This does not block a free-text `status` column; only the write-API validation and the UI copy (`PLAN-09`, `PLAN-14`) depend on the final names.
2. **Mandatory-field list.** Which columns are required on save is an open question tied to the row layout and field set on the mockup (`overview` open question 6, `PLAN-10` AC2). The schema stays permissive (most fields nullable) on purpose so the required-field set is enforced at the write boundary (`PLAN-09`) once settled, rather than baked into the columns now.
3. **Keeping `updated_at` fresh.** The table matches `users` and sets `updated_at` through `$defaultFn` on insert only. Whether to add a Drizzle `$onUpdate(() => new Date())` or have the `PLAN-09` write API set `updated_at` on every mutation is left to the backend stage; the default assumption is to match the existing `users` pattern and have the write API touch it.
