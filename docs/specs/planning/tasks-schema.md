...
1. **Status vocabulary (soft, `overview` open question 5).** The three trackable status names are `Accepté` / `En cours` / `Terminé`, with `N/A` for non-trackable, still to be confirmed with the user. This does not block a free-text `status` column; only the write-API validation and the UI copy (`PLAN-09`, `PLAN-14`) depend on the final names.
