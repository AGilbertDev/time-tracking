...
- **`words_translated(period)`** is the sum of each trackable task's own word total across the period, read from `project_word_count`. That column holds the words for that row's own day, a stored fact rather than a whole project's total, and **splitting is what keeps the two the same thing**. A piece of work spanning several days is several rows, each carrying the words actually done that day, so no day can claim a whole project and the sum stays honest. See [tasks, splits, recurrence](#tasks-splits-recurrence).
  - **Corrected 2026-07-30.** This bullet used to define the numerator as the sum of `words_done`, which was the second word column on a task. `PLAN-33` dropped that column, so the definition is rewritten rather than annotated, because it is the formula a future implementer builds from rather than a record of a decision. The model did not change and only the column did. The reasoning is in [words are a total](#words-are-a-total-not-a-progress-pair-deferred-to-a-later-feature) and the drop is specced in [row-simplification-words-total.md](row-simplification-words-total.md). One consequence worth carrying, since it is the whole reason the old column existed. The guard against a multi-day job counting its whole total on every day it touches now lives in the shape of the rows rather than in a second column, so it depends on `PLAN-18` existing, which makes `PLAN-18` before `PLAN-22` an ordering constraint rather than a preference.
...
Given 2026-07-29. **Implemented 2026-07-30 by `PLAN-33`**, specced in [row-simplification-words-total.md](row-simplification-words-total.md). The heading keeps the word "deferred" because that is what this section recorded at the time and the anchor is linked from several documents. The column is gone, the row prints one figure, and the paragraphs below are the reasoning that led there rather than pending work. The user's words: "we don't track words translated in realtime. useless to track x/y. only totals."
...
One consequence to carry into `PLAN-33`, now answered. `project_word_count` becomes a misleading name once a row holds a slice total rather than a whole project's, and that feature had to decide whether to rename it in the same migration or leave it with a comment. **It left the name and added the comment**, because a rename is a contract change where the drop is not, and because the name is not yet misleading while `PLAN-18` remains unbuilt. `PLAN-18` is the trigger to revisit it and `word_count` is the recommended target.
...
- Editing a **past** task's `words_done`, `actual_minutes`, or category **does** restate that past period. That is a correction of the record, which is intended, not the bug we guard against.
  - **Annotated 2026-07-30, and the rule is unchanged.** `words_done` was dropped by `PLAN-33` in migration `0008`. Read the field as `project_word_count`, which is now the only word column on a task. The sentence is left as written because it records a decision that was correct at the time, and what it decided is untouched. Editing a past task's stored facts still restates that past period on purpose.
...
- **Annotated 2026-07-30. The two bullets above are the locked model and it stands. Only the column name in them is gone.** `words_done` was dropped by `PLAN-33` in migration `0008`, specced in [row-simplification-words-total.md](row-simplification-words-total.md), so read every `words_done` above as `project_word_count`. Each slice still carries the words actually done that day as its own total, the slices still share a `project` and a `split_group_id`, and the quota still attributes each day only its own words. Both bullets are left as written because they record a decision that was correct when made and that this feature did not reverse. What changed is that the per-day figure and the task's own word total became the same column instead of two, which is why the model reads as simpler now rather than different.
  - **One sentence in the split bullet aged well and is worth pulling out**, because it anticipated a ruling made much later. It says equal distribution over the span is offered only as an editable pre-fill and never as the reported number, since it is wrong when effort is uneven. The owner reached the same conclusion independently on 2026-07-30 when he said the split feature will ask the user how many words each part gets, which is now `PLAN-18`'s `AC4`. The dev seed does divide its split pairs equally, and that is reference data with no user to ask rather than the model.
...
Depends on `PLAN-32`. Backend and frontend, plus a migration, plus a narrow design pass.

**Implemented**, specced in [row-simplification-words-total.md](row-simplification-words-total.md).

**The stage line above is corrected.** It read "Frontend, plus a design pass", which was true while this was a display change and stopped being true the same day, when the scope grew to include the schema and a real migration. Backend leads, and the design pass is the narrowest stage rather than the leading one. Compliance runs too, briefly, because the feature irreversibly drops a column from a table holding client and project names, and a checked verdict on that is worth more than an assumed one.

**The `project_word_count` name question is answered. It keeps its name for now, with a comment.** The decisive argument is that a rename is a contract change where the drop is not, since `projectWordCount` is client-writable on both `PLAN-09` endpoints and both schemas are strict, so renaming it breaks a key `PLAN-10` and `PLAN-11` will send. The name is also not yet misleading, because no row can be a slice until `PLAN-18` exists. `PLAN-18` is the trigger for the rename, `word_count` is the recommended target, and the full tradeoff is in the spec.

**The `PLAN-32a` no-history finding expired when `PLAN-09` landed on 2026-07-30, so this migration is not the free one `32a` got.** That finding said no real user task history existed, and it rested on there being no task write path. `PLAN-09` is that write path, so from the day it shipped the `tasks` table holds rows the user created. Dropping `words_done` is now a migration against a table with real user rows in it rather than against one only the seed has ever written. Re-run the row check rather than inheriting it from this document or from [nine-task-categories.md](nine-task-categories.md), and run it against production rather than against a fresh dev database, since a dev database can be seed-only while production is not. The reasoning is in [task-write-api.md](task-write-api.md).
...
**`AC1` returns 404 rather than 403, corrected in the bullet itself on 2026-07-30.** The guarantee is unchanged, and a user still cannot mutate another user's task. Only the reported status differs. A 403 on someone else's task id confirms that the id exists, so a 404 for the missing case and the not-yours case alike leaks nothing and meets the same guarantee more safely. This note is kept as the reasoning for the change rather than as a warning about a contradiction, because the criterion below now says 404 and there is no longer a weaker version of it for anyone to follow.

- Create, update, and delete, each validated with Zod, logic in `handlers/`, ownership enforced.
- AC1. A user can only mutate their own tasks, and a missing or inaccessible row returns 404 rather than 403, so the status code is not an enumeration oracle. AC2. Invalid bodies return a structured 422 via `sendZodError`. AC3. Thin route files, handlers extracted.
...
**Amended 2026-07-30 by `PLAN-33`, which dropped the column the three criteria above are written against.** They still describe the right behaviour and they name a field that no longer exists, so read them through this note rather than literally. There is no `words_done` any more. Each slice carries its own words in `project_word_count`, as its own total for its own day, and the group sums to the job. So AC1 becomes that each slice holds the words for its own day, and AC3 becomes that the quota sums row totals and therefore attributes each day only its own. The reasoning is in [words are a total](#words-are-a-total-not-a-progress-pair-deferred-to-a-later-feature) and the drop is specced in [row-simplification-words-total.md](row-simplification-words-total.md).

**This feature is now what protects the quota numerator, which raises what it was.** The dropped column existed to stop a multi-day job counting its whole total on every day it touched, and splitting is what replaces that guard. Until this feature exists, a genuinely multi-day job entered as one row attributes the whole job to one day. Nothing reads the numerator yet, because `PLAN-22` is not built, so the window is currently harmless. It stops being harmless if `PLAN-22` lands first, so `PLAN-18` before `PLAN-22` is an ordering constraint rather than a preference.

**`PLAN-18` asks the user how many words each part gets. It does not divide automatically.** Decided by the owner on 2026-07-30, in his words, "seed can split equal, but the split feature will later ask how many words each part". Only the user knows how far they actually got on the first day, and a job spread over two days is almost never spread evenly, so an automatic division would be the app inventing a figure the user is the only source for. That is the same defect that killed the words pair in the first place, which was a number the app would end up guessing, so this feature must not reintroduce it in a new shape.

- AC4, added here. The split flow collects the per-slice word count from the user rather than computing it, and the slices sum to the job total.

**The dev seed divides its split pairs equally, and that is reference data rather than the model.** `scripts/seed.ts` gives each slice of its seeded pairs half the job total, with any odd remainder on the first slice so the pair sums exactly. It does that because it needs plausible rows and has no user to ask, not because halving is the rule. Nobody reading the seed should infer that equal division is the product behaviour, and nobody building this feature should reach for an automatic halving because the seed does one.
...
**One ordering constraint follows and it is not a preference.** The guard is `PLAN-18`'s to create, so building this engine before splitting exists means a genuinely multi-day job entered as one row attributes the whole job to a single day, with nothing to catch it. `PLAN-18` comes first. The dependency line above already names `PLAN-18`, and this is why it is there.
