...
**Delivered 2026-07-31 by [task-inline-editor.md](task-inline-editor.md), which built it in the same pass as `PLAN-10` and `PLAN-11`.** Doing all three together was the cheaper order, because the editor renders whatever the schema holds and building it before the column went would have meant building the words field twice. The two things this entry asked the implementing feature to settle are settled there. `project_word_count` is **not** renamed, because a rename means either an `ALTER TABLE RENAME COLUMN` plus an edit to every reader or a create-copy-swap, and it also renames `projectWordCount` on a request contract that had already shipped, which turns an internal tidy into a breaking API change for no behaviour gained; the fix is a comment on the column instead. And a task that is not split loses nothing, because for a single-day job the row total and the project total are the same number, which is the common case and stays the simple one.

The migrations are `0008_drop_tasks_words_done.sql` and `0009_add_tasks_notes.sql`, both written by hand and both applied to the development database on 2026-07-31. **The production row check below is still open and still needs the owner**, because there are no production credentials in the container this was built in.
...
**`AC1` annotated 2026-07-31 by [task-inline-editor.md](task-inline-editor.md), because the inherited list changed by one member in each direction.** `Mots faits` came off it. The handoff called that field "the quota numerator and the single most important stored number in the app", which was true of an intention rather than of the code, since no quota engine exists yet and nothing read the column for a statistic. The owner killed it for a reliability reason rather than a simplicity one, "on ne track pas le nb de mots vs le total. seulement le total. sinon on n'aura jamais des stats fiables", and a figure the user will not reliably enter produces worse statistics than no figure. The write API already refused it in both bodies, so a field for it would have had nowhere to send its value, and migration `0008` dropped the column it wrote to. The cost is real and is recorded in that spec, which is that partial progress inside one row can no longer be expressed and is expressed as one row per day instead, by hand until `PLAN-18` ships. A `Notes` field went on in its place, backed by the new `notes` column from migration `0009`, and it is a fresh field rather than a revival of `instructions`. `AC3` is also confirmed rather than still open, and the answer is yes, one editor at a time across the whole week.

**PLAN-12 — Estimated duration auto-calc and actual auto-sync**
Depends on PLAN-11. Backend, plus shared for any pure rounding rule both sides need.

- `estimated_minutes = round_to_5(words / quota × 60)`, stored frozen.
- **Corrected 2026-07-31. This entry read "Frontend plus shared" and that assignment was wrong.** It put the derivation in the client, which contradicts the standing convention that a derived value is resolved on the server and arrives finished rather than being shipped as a raw row plus the rules for reading it. The derivation belongs in the server write handlers. **No component may compute an estimate from a word count and a quota**, and [task-inline-editor.md](task-inline-editor.md) already carries that as a negative acceptance criterion, that a search for a division by a quota under `app/` returns nothing. If a pure rounding helper is genuinely needed on both sides it lives once in `shared/planning.ts` and both import it, which is the single acceptable form of sharing a pure rule, rather than being copied. The formula and the acceptance examples below were always right and are unchanged.
- **The `actual_minutes` mirror is a second correction, and this one is a trap rather than a mislabel.** This entry used to say `actual_minutes` mirrors estimated until the user edits it. Do not build that. Storing the copy makes a duration the user confirmed and a duration the app assumed into identical rows, and nothing downstream can tell them apart afterwards, so the fallback is resolved at read time by `effectiveDuration` and the column stays NULL until the user measures something. That was settled by [task-write-api.md](task-write-api.md) and it is already enforced in shipped code, so a stage following the old wording would write the mirror and then be failed by guard tests in three files that exist specifically to catch it. A `NULL` actual and an actual of `0` also have to stay distinguishable, since zero is a measurement and NULL is the absence of one, which is the only way back from a wrong duration to unmeasured.
...
**Annotated 2026-07-31, because [task-inline-editor.md](task-inline-editor.md) dropped `words_done` and the bullet and criteria below still name it.** A reader following them would go looking for a column that no longer exists. The mechanism is unchanged and only the field it is described in terms of is wrong, so read them this way instead. Splitting sets the current row's `project_word_count` to the words actually done that day, then creates a sibling row carrying the remainder, and the two are joined by `split_group_id`. The quota attributes each day its own row total. That is exactly the model `PLAN-33` decided on 2026-07-29, so nothing new is invented here.

- Record the words actually done on the current day and port the remainder to a chosen day as a linked task sharing the project, joined by `split_group_id`. (Originally worded as recording `words_done`, a column migration `0008` dropped.)
- AC1. The current day keeps its own `project_word_count`, the new day carries the remainder of the project total. AC2. Both tasks share the project and the split group. AC3. The quota attributes each day only its own row total. (`AC1` and `AC3` originally named `words_done`.)
...
**Annotated 2026-07-31, because the tenth category filled the palette and that constrains `AC5`.** Placing `other` measured the best remaining slot at 0.0493 from its nearest neighbour, against 0.0469 for the closest pair already shipped. The margin over the next-tightest pair is therefore about five percent rather than the twenty percent earlier additions had, so a freely chosen hue is no longer reliably distinct from the ten in use. **If this feature lets a user pick a category colour, it has to suggest a hue or warn on collision rather than let someone choose blind.** Picking blind from a full wheel produces two categories the user cannot tell apart, and the failure is quiet, because each colour looks right on its own and only the pair is wrong. Note that this is a different question from the contrast note earlier in this document. Contrast really is free for a new hue, since every angle clears the floor at the fixed lightness and chroma, but distinctness is not, and a reader who takes the first as covering the second will ship a collision.

The narrow precedent matters as much as the number, because the number alone teaches the wrong lesson. The tighter spacing was accepted for `other` in particular, and not because 0.0493 is a comfortable margin. `Autre` is the shortest label of the ten and shares no prefix with any other, so colour does almost no scanning work on it. `Révision interne` against `Révision externe` is the opposite case, where the labels are nearly identical and colour carries the disambiguation, and five percent would not be acceptable there. So read this as a decision about one label rather than as clearance for an eleventh hue. The full working is in [other-category-design.md](other-category-design.md).
